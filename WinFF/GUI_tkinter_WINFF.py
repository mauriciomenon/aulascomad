...
tk.Button(root, text="Procurar", command=select_output_directory).grid(row=1, column=2, padx=10, pady=5)

# Formato de saída
tk.Label(root, text="Selecione o Formato de Saída:").grid(row=2, column=0, padx=10, pady=5)
...
# Bitrate de vídeo
tk.Label(root, text="Bitrate de Vídeo (ex.: 1000k):").grid(row=3, column=0, padx=10, pady=5)
video_bitrate_entry = tk.Entry(root, width=20)
video_bitrate_entry.grid(row=3, column=1, padx=10, pady=5)

# Bitrate de áudio
tk.Label(root, text="Bitrate de Áudio (ex.: 128k):").grid(row=4, column=0, padx=10, pady=5)
audio_bitrate_entry = tk.Entry(root, width=20)
audio_bitrate_entry.grid(row=4, column=1, padx=10, pady=5)

# Resolução
tk.Label(root, text="Selecione a Resolução:").grid(row=5, column=0, padx=10, pady=5)
...
# Codec de vídeo
tk.Label(root, text="Selecione o Codec de Vídeo:").grid(row=6, column=0, padx=10, pady=5)
...
# Codec de áudio
tk.Label(root, text="Selecione o Codec de Áudio:").grid(row=7, column=0, padx=10, pady=5)
...
# Caminho do FFmpeg
tk.Label(root, text="Caminho do Executável FFmpeg:").grid(row=8, column=0, padx=10, pady=5)
ffmpeg_path_entry = tk.Entry(root, width=50)
ffmpeg_path_entry.insert(0, config.get('DEFAULT', 'ffmpeg_path', fallback='ffmpeg'))
ffmpeg_path_entry.grid(row=8, column=1, padx=10, pady=5)
tk.Button(root, text="Procurar", command=select_ffmpeg_executable).grid(row=8, column=2, padx=10, pady=5)

# Botão para converter vídeo
convert_button = tk.Button(root, text="Converter", command=convert_video)
convert_button.grid(row=10, column=0, columnspan=3, pady=20)
